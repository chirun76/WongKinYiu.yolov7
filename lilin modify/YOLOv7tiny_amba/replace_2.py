...
def create_constant_4_initializer(output_name):
    ...
    for i in range(40):
        row = []
        for j in range(40):
            first_col_value = (j * 16) - 8
            second_col_value = (i * 16) - 8
            row.append([first_col_value, second_col_value])
        values.append(row)
    values = np.array(values, dtype=np.float32).reshape([1, 1, 1600, 2])
...
def create_test_output_graph(new_graph_name):
    print("====== Creating Conv_output_0-to-Reshape_3_output_0 Replacement Graph with Mul and Add Nodes ======")
    
    new_nodes = []
    initializers = []

    new_inputs = [helper.make_tensor_value_info(
        '/model/model.77/m.1/Conv_output_0', TensorProto.FLOAT, [1, 255, 40, 40])]
    new_outputs = [helper.make_tensor_value_info(
        '/model/model.77/Reshape_3_output_0', TensorProto.FLOAT, [1, 4800, 85])]

    new_nodes.extend(create_reshape_node('/model/model.77/Reshape_2_output_0', '/model/model.77/m.1/Conv_output_0', [1, 3, 85, 1600]))
    new_nodes.append(create_transpose_node('/model/model.77/Transpose_1_output_0', '/model/model.77/Reshape_2_output_0', [0,1,3,2]))

    sigmoid_node = helper.make_node(
        op_type='Sigmoid',
        inputs=['/model/model.77/Transpose_1_output_0'],
        outputs=['/model/model.77/Sigmoid_1_output_0'],
        name='/model/model.77/Sigmoid_1'
    )
    new_nodes.append(sigmoid_node)

    split_output_names = ['/model/model.77/Split_1_output_0', '/model/model.77/Split_1_output_1', '/model/model.77/Split_1_output_2']
    split_node = create_split_node(split_output_names, '/model/model.77/Sigmoid_1_output_0')
    new_nodes.append(split_node)

    constant_tensor = numpy_helper.from_array(np.array(32, dtype=np.float32), name='/model/model.77/Constant_9_output_0')
    initializers.append(constant_tensor)

    mul_node = helper.make_node( 
        op_type='Mul',
        inputs=['/model/model.77/Split_1_output_0', '/model/model.77/Constant_9_output_0'],
        outputs=['/model/model.77/Mul_2_output_0'],
        name='/model/model.77/Mul_2'
    )
    new_nodes.append(mul_node)

    add_constant_tensor = create_constant_4_initializer('/model/model.77/Constant_10_output_0')
    initializers.append(add_constant_tensor)

    add_node = helper.make_node(
        op_type='Add',
        inputs=['/model/model.77/Mul_2_output_0', '/model/model.77/Constant_10_output_0'],
        outputs=['/model/model.77/Add_1_output_0'],
        name='/model/model.77/Add_1'
    )
    ...
    pow_node = create_pow_node('/model/model.77/Pow_1_output_0', '/model/model.77/Split_1_output_1', '/model/model.77/Constant_5_output_0')
    new_nodes.append(pow_node)

    constant_6_tensor = create_constant_initializer('/model/model.77/Constant_12_output_0', np.array([[[[144, 300]], [[304, 220]], [[288, 584]]]], dtype=np.float32))
    initializers.append(constant_6_tensor)

    mul_node_after_pow = helper.make_node(
        op_type='Mul',
        inputs=['/model/model.77/Pow_1_output_0', '/model/model.77/Constant_12_output_0'],
        outputs=['/model/model.77/Mul_3_output_0'],
        name='/model/model.77/Mul_3'
    )
    new_nodes.append(mul_node_after_pow)

    concat_node = helper.make_node(
        op_type='Concat',
        inputs=['/model/model.77/Add_1_output_0', '/model/model.77/Mul_3_output_0', '/model/model.77/Split_1_output_2'],
        outputs=['/model/model.77/Concat_1_output_0'],
        name='/model/model.77/Concat_1',
        axis=3
    )
    new_nodes.append(concat_node)

    new_nodes.extend(create_reshape_node('/model/model.77/Reshape_3_output_0', '/model/model.77/Concat_1_output_0', [1, -1, 85]))
...
if __name__ == '__main__':
    create_test_output_graph('replace_subgraph_2')
