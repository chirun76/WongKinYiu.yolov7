...
def create_constant_4_initializer(output_name):
    ...
    for i in range(80):
        row = []
        for j in range(80):
            first_col_value = (j * 8) - 4
            second_col_value = (i * 8) - 4
            row.append([first_col_value, second_col_value])
        values.append(row)
    values = np.array(values, dtype=np.float32).reshape([1, 1, 6400, 2])
...
def create_test_output_graph(new_graph_name):
    print("====== Creating Conv_output_0-to-Reshape_1_output_0 Replacement Graph with Mul and Add Nodes ======")
    
    new_nodes = []
    initializers = []

    new_inputs = [helper.make_tensor_value_info(
        '/model/model.77/m.0/Conv_output_0', TensorProto.FLOAT, [1, 255, 80, 80])]
    new_outputs = [helper.make_tensor_value_info(
        '/model/model.77/Reshape_1_output_0', TensorProto.FLOAT, [1, 19200, 85])]

    new_nodes.extend(create_reshape_node('/model/model.77/Reshape_output_0', '/model/model.77/m.0/Conv_output_0', [1, 3, 85, 6400]))
    new_nodes.append(create_transpose_node('/model/model.77/Transpose_output_0', '/model/model.77/Reshape_output_0', [0,1,3,2]))

    sigmoid_node = helper.make_node(
        op_type='Sigmoid',
        inputs=['/model/model.77/Transpose_output_0'],
        outputs=['/model/model.77/Sigmoid_output_0'],
        name='/model/model.77/Sigmoid'
    )
    new_nodes.append(sigmoid_node)

    split_output_names = ['/model/model.77/Split_output_0', '/model/model.77/Split_output_1', '/model/model.77/Split_output_2']
    split_node = create_split_node(split_output_names, '/model/model.77/Sigmoid_output_0')
    new_nodes.append(split_node)

    constant_tensor = numpy_helper.from_array(np.array(16, dtype=np.float32), name='/model/model.77/Constant_3_output_0')
    initializers.append(constant_tensor)

    mul_node = helper.make_node( 
        op_type='Mul',
        inputs=['/model/model.77/Split_output_0', '/model/model.77/Constant_3_output_0'],
        outputs=['/model/model.77/Mul_output_0'],
        name='/model/model.77/Mul'
    )
    new_nodes.append(mul_node)

    add_constant_tensor = create_constant_4_initializer('/model/model.77/Constant_4_output_0')
    initializers.append(add_constant_tensor)

    add_node = helper.make_node(
        op_type='Add',
        inputs=['/model/model.77/Mul_output_0', '/model/model.77/Constant_4_output_0'],
        outputs=['/model/model.77/Add_output_0'],
        name='/model/model.77/Add'
    )
    ...
    pow_node = create_pow_node('/model/model.77/Pow_output_0', '/model/model.77/Split_output_1', '/model/model.77/Constant_5_output_0')
    new_nodes.append(pow_node)

    constant_6_tensor = create_constant_initializer('/model/model.77/Constant_6_output_0', np.array([[[[48, 64]], [[76, 144]], [[160, 112]]]], dtype=np.float32))
    initializers.append(constant_6_tensor)

    mul_node_after_pow = helper.make_node(
        op_type='Mul',
        inputs=['/model/model.77/Pow_output_0', '/model/model.77/Constant_6_output_0'],
        outputs=['/model/model.77/Mul_1_output_0'],
        name='/model/model.77/Mul_1'
    )
    new_nodes.append(mul_node_after_pow)

    concat_node = helper.make_node(
        op_type='Concat',
        inputs=['/model/model.77/Add_output_0', '/model/model.77/Mul_1_output_0', '/model/model.77/Split_output_2'],
        outputs=['/model/model.77/Concat_output_0'],
        name='/model/model.77/Concat',
        axis=3
    )
    new_nodes.append(concat_node)

    new_nodes.extend(create_reshape_node('/model/model.77/Reshape_1_output_0', '/model/model.77/Concat_output_0', [1, -1, 85]))
...
if __name__ == '__main__':
    create_test_output_graph('replace_subgraph_1')
